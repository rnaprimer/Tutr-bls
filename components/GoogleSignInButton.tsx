...
export function GoogleSignInButton({
  ...
  const handleSignIn = async () => {
    try {
      ...
      setErrorMessage("Could not connect to Google authentication. Please try again.");
      setLoading(false);
    }
  };
  ...
  return (
    <div className="w-full">
      {errorMessage && (
        <div
          role="alert"
          className="mb-4 p-3.5 rounded-xl bg-red-50 border border-red-200 text-xs text-red-700 font-medium leading-relaxed"
        >
          {errorMessage}
        </div>
      )}
