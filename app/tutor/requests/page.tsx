...
export default async function TutorRequestsPage() {
  ...
  // If user is not yet a verified tutor, guide them back to the tutor portal status view
  if (!tutorProfile || !tutorProfile.is_verified) {
    return (
      <div className="min-h-screen bg-beige-light/30 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-3xl border border-navy/10 p-8 text-center shadow-sm">
          ...
          <Link
            href="/tutor"
            className="inline-flex items-center justify-center gap-2 px-6 py-2.5 rounded-full bg-navy text-white text-xs font-semibold hover:bg-navy-dark transition-colors"
          >
      ...
      <header className="w-full bg-white/95 border-b border-navy/10 px-4 sm:px-6 lg:px-8 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <Link
              href="/tutor"
              className="text-xs font-semibold text-navy/70 hover:text-navy px-3.5 py-1.5 rounded-full hover:bg-beige/60 transition-colors"
            >
              My Application
            </Link>
            <SignOutButton />
      ...
      <main className="flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <div className="inline-flex items-center gap-1.5 text-xs font-semibold text-navy/60 mb-1">
              <Link href="/tutor" className="hover:text-navy">
                Tutor Portal
              </Link>
              <span>/</span>
            ...
            <p className="text-xs text-navy/60 mt-1">
              Welcome, <span className="font-semibold text-navy">{tutorProfile.display_name}</span>. Review and respond to student inquiries.
            </p>
          </div>

          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-semibold bg-emerald-100 text-emerald-900 border border-emerald-200 self-start sm:self-auto">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-2 gap-4 mb-8">
          <div className="bg-white rounded-2xl border border-navy/10 p-5 shadow-sm">
            <span className="text-xs font-bold uppercase tracking-wider text-navy/50 block mb-1">
              Pending Requests
            </span>
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-xs text-navy/50">waiting for response</span>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl border border-navy/10 p-5 shadow-sm">
            ...
            <div className="flex items-baseline gap-2">
              ...
              <span className="text-xs text-navy/50">all-time received</span>
            </div>
          </div>
        </div>
              ...
              return (
                ...
                >
                  <div className="space-y-2">
                    ...
                    {req.message && (
                      <div className="p-3 rounded-xl bg-beige-light/70 border border-navy/5 text-xs text-navy/80">
                        <span className="text-[10px] font-bold uppercase tracking-wider text-navy/40 block mb-0.5">
                          Student Note
                        </span>
                    ...
                    {req.status === "ACCEPTED" && (
                      <div className="mt-3 p-3.5 rounded-2xl border text-xs space-y-2 bg-sky/20 border-sky/30">
                        <div className="flex items-center justify-between">
                          <span className="font-bold text-navy">Request Accepted</span>
                        ...
                        {req.connection?.status === "CONTACT_UNLOCKED" && req.unlockedStudent ? (
                          <div className="p-3 bg-white rounded-xl border border-emerald-100 space-y-2 mt-1">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 text-xs">
                              {req.unlockedStudent.email && (
                                <div>
                                  <span className="text-[10px] uppercase font-bold text-navy/40 block">Email Address</span>
                                  <a
                                    href={`mailto:${req.unlockedStudent.email}`}
                                    className="font-semibold text-navy hover:text-teal underline"
                                  >
                                    {req.unlockedStudent.email}
                                  </a>
                                </div>
                              ...
                              <div>
                                <span className="text-[10px] uppercase font-bold text-navy/40 block">Phone Number</span>
                                {req.unlockedStudent.phone ? (
                                  ...
                                  <span className="text-navy/50 italic">Not provided</span>
                                )}
                              </div>
                            </div>
                          </div>
                  ...
                  <div className="self-end sm:self-center">
                    {req.status === "PENDING" ? (
                      ...
                      <div className="text-right text-[11px] text-navy/40">
                        {req.responded_at ? (
                          <span>
                            Responded on{" "}
